...
func TestRequestReport(t *testing.T) {
	...
	xml, err := _apiSet.Reports.RequestReport(&reports.RequestReportQuery{
		...
		StartDate:  date.Add(-24 * 30 * time.Hour).Format(time.RFC3339),
		EndDate:    date.Add(-24 * 0 * time.Hour).Format(time.RFC3339),
	})
